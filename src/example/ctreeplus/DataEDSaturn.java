...
*/
public class DataEDSaturn extends DoubleVectorHolderSaturn
 ...
 */
public DataEDSaturn(String prefix) {
	super(prefix); 
...
public void nextCycle( Node node, int pid )
{
	...
	if (sendertype.getType()==0){
		...
		for (int i=0; i<linkable.degree(); i++){
			...
			if ((getEpoch() % ntype.getFrequency(node.getID())) == 0 ){
				if(!peern.isUp()) return;
				PriorityQueue<EventUID> queueToSend = queue.get(peern.getID());
				//System.out.println("Sending queue "+queueToSend+" to node "+peern.getID()+" from node "+node.getID());
				if (queueToSend != null){
					PriorityQueue<EventUID> clonedQueue = clonePriorityQueue(queueToSend);
					((Transport)node.getProtocol(FastConfig.getTransport(pid))).
					send(node, peern, new QueueMessage(clonedQueue, ntype.getType(), getEpoch(), node.getID()), pid);
...
//--------------------------------------------------------------------------
	...
	class QueueMessage {
		...
		final PriorityQueue<EventUID> queue;
		final int type;
		final int epoch;
		final long senderId;

		public QueueMessage(PriorityQueue<EventUID> queue, int type, int epoch, long senderId)
		{
			this.queue = queue;
			this.type = type;
			this.epoch = epoch;
